        ...
        <?php foreach($data['all-admin'] as $i => $admin ) : ?>
            <tr class="text-center">
                ...
                <td>
                    ...
                    <?php elseif($admin['status'] == 'unactive') : ?>
                        <span class="text-danger fw-bold">Tidak Aktif</span>
                    <?php endif; ?>
                </td>
                <td>
                    <a href="<?= url('admin/edit/'.$admin['id_admin']) ?>" class="btn btn-warning text-white"><i class='bx bx-edit'></i></a>
                    <a href="" class="btn btn-danger text-white" data-bs-toggle="modal" data-bs-target="#deleteAdminModal<?= $admin['id_admin'] ?>"><i class='bx bxs-trash-alt'></i></a>


                        <!-- Modal -->
                        <div class="modal fade" id="deleteAdminModal<?= $admin['id_admin'] ?>" tabindex="-1" aria-labelledby="deleteAdminModalLabel" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                <form action="<?= url('admin/delete/'.$admin['id_admin']) ?>" method="post">
